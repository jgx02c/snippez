/**
 * @file This routes requests for the Department table.
...
/*
 * This processes a request for one record from the Department table.
 *
 */
    ...
    if( typeof parameter === 'number' && !( Number.isNaN( parameter ) ) ) {
      ...
      if( Number.isInteger( parameter ) ) {
        ...
        if( parameter > 0 ) {
          ...
          response.json( await department.getOne( parameter ) );
...
/*
 * This processes a request for many records from the Department table.
 *
 */
    ...
    if( typeof parameter === 'number' && !( Number.isNaN( parameter ) ) ) {
      ...
      if( Number.isInteger( parameter ) ) {
        ...
        if( parameter > 0 ) {
          ...
          response.json( await department.getPage( parameter ) );
        // The number is an integer equal to zero.
        } else if ( parameter == 0 ) {
          // We get all records.
          response.json( await department.get() );
