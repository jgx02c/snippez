/**
 * @file This routes requests for the Doctor table.
...
/*
 * This processes a request for one record from the Doctor table.
 *
 */
    ...
    if( typeof parameter === 'number' && !( Number.isNaN( parameter ) ) ) {
      ...
      if( Number.isInteger( parameter ) ) {
        ...
        if( parameter > 0 ) {
          ...
          response.json( await doctor.getOne( parameter ) );
...
/*
 * This processes a request for many records from the Doctor table.
 *
 */
    ...
    if( typeof parameter === 'number' && !( Number.isNaN( parameter ) ) ) {
      ...
      if( Number.isInteger( parameter ) ) {
        ...
        if( parameter > 0 ) {
          ...
          response.json( await doctor.getPage( parameter ) );
        // The number is an integer equal to zero.
        } else if ( parameter == 0 ) {
          // We get all records.
          response.json( await doctor.get() );
